...
export const Route = createFileRoute("/platform/workforce-announcements")({
  head: () => ({ meta: [{ title: "Workforce Announcements — Parther Admin" }] }),
  component: WorkforceAnnouncementsPage,
});

type FormState = { title: string; body: string; expiresAt: string };
const INIT: FormState = { title: "", body: "", expiresAt: "" };

function WorkforceAnnouncementsPage() {
  ...
  const openEdit = (a: any) => {
    setEditId(a.id);
    setForm({ title: a.title, body: a.body, expiresAt: a.endsAt?.slice(0, 10) ?? "" });
    setOpen(true);
  };
  ...
  const workforceAnnouncements = ((announcements as any[]) || []).filter(a => a.target === "WORKFORCE");

  return (
    <div>
      <PageHeader
        title="Workforce Announcements"
        description="Broadcast announcements to the workforce app."
        actions={
      ...
      <div className="p-6 space-y-4">
        ...
        ) : workforceAnnouncements.length ? (
          workforceAnnouncements.map((a) => (
            <Card key={a.id}>
                ...
                <div className="flex gap-3">
                  ...
                  <div>
                    <div className="flex items-center gap-2 flex-wrap">
                      ...
                      <Badge variant="outline" className="text-primary border-primary/40">Workforce</Badge>
